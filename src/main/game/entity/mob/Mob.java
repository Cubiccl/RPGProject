...
public class Mob extends Entity {
  ...
  public Mob(int health, float speed, float x, float y, Animation animation) {
    ...
    this.xMotion = 0;
    this.yMotion = 0;
    this.movementSprites = new Animation[] {animation, animation, animation, animation};
    this.standingSprites = new Animation[] {animation, animation, animation, animation};
  }

  public void setMovementAnimations(Animation downSprite, Animation upSprite, Animation leftSprite,
      Animation rightSprite) {
  ...
  public void setStandingAnimations(Animation downSprite, Animation upSprite, Animation leftSprite,
      Animation rightSprite) {
  ...
  public void move(Map map) {
    ...
    if (!(map.isSolid(nextX, this.y) || map.isSolid(nextX + 1, this.y)
        || map.isSolid(nextX, this.y + 1) || map.isSolid(nextX + 1, this.y + 1)))
      this.x = nextX;

    if (!(map.isSolid(this.x, nextY) || map.isSolid(this.x + 1, nextY)
        || map.isSolid(this.x, nextY + 1) || map.isSolid(this.x + 1, nextY + 1)))
      this.y = nextY;
  }
}
