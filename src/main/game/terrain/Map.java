...
public class Map {
   ...
   * 
   * @param tile the tile id you want on the map
   */
  ...
  public void initFrom(String filePath) {
    ...
    for (int x = 0; x < this.actual_size; x++) {
      for (int y = 0; y < this.actual_size; y++) {
        this.background[x][y] = (short) Utils.parseInt(back[this.actual_size * y + x]);

        this.midground[x][y] = (short) Utils.parseInt(mid[this.actual_size * y + x]);

        this.foreground[x][y] = (short) Utils.parseInt(fore[this.actual_size * y + x]);
      }
    }
  ...
  public void setTileAt(Tile tile, byte layer, int x, int y) {
    if (x < 0 || y < 0 || x >= this.background.length || y >= this.background.length) {
  ...
  /**
   * resizes the map to the desired value. Use tile 0 as default for more optimized usage.
   * 
   * @param newsize the new size
   * @param tile the id of the tile you want.
   * 
   * */
   ...
   * 
   * @throws dimensionNotSquarredExeption make sure your dimension object is a square, cause the map
   *         is always a square.
   * */
  ...
  public void render(Graphics g) {
    ...
    for (int x = (int) Math.floor(startX / Tile.WIDTH); x < endX; x++) {
      for (int y = (int) Math.floor(startY / Tile.HEIGHT); y < endY; y++) {
        Tiles.getTileFromId(this.background[x][y]).renderAt(g, x * Tile.WIDTH - (int) startX,
            y * Tile.HEIGHT - (int) startY);

        if (this.midground[x][y] != 0)
          Tiles.getTileFromId(this.midground[x][y]).renderAt(g, x * Tile.WIDTH - (int) startX,
              y * Tile.HEIGHT - (int) startY);
      }
    }
    ...
    for (int x = (int) Math.floor(startX / Tile.WIDTH); x < endX; x++) {
      for (int y = (int) Math.floor(startY / Tile.HEIGHT); y < endY; y++) {
        if (this.foreground[x][y] != 0)
          Tiles.getTileFromId(this.foreground[x][y]).renderAt(g, x * Tile.WIDTH - (int) startX,
              y * Tile.HEIGHT - (int) startY);
      }
    }
  }
  ...
  public boolean isSolid(float x, float y) {
    ...
    if (xFloor < 0 || xFloor >= this.actual_size || yFloor < 0 || yFloor >= this.actual_size)
      return false;

    return Tiles.getTileFromId(this.getTileAt(BACKGROUND, xFloor, yFloor)).isSolid()
        || Tiles.getTileFromId(this.getTileAt(MIDGROUND, xFloor, yFloor)).isSolid();
  }
