...
public abstract class MenuState extends State {
  ...
  public void render(Graphics g) {
    ...
    for (int i = 0; i < size - 1; i++) {
      ...
      if (this.selected == i)
        g.setColor(SELECTED);
      g.drawString(this.buttons.get(i).getText(),
          width / 2 - metrics.stringWidth(this.buttons.get(i).getText()) / 2, height * (i + 1)
              / size);
    }
  ...
  public void update() {
    ...
    if (RPGProject.getGame().getKeyManager().isKeyPressedInstant(KeyCustom.up.getKeyCode()))
      this.selected--;
    if (RPGProject.getGame().getKeyManager().isKeyPressedInstant(KeyCustom.down.getKeyCode()))
      this.selected++;
    ...
    if (RPGProject.getGame().getKeyManager().isKeyPressedInstant(KeyCustom.enter.getKeyCode()))
